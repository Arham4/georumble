...
| `preview-video-640x360.mp4` | Video Preview (Activities → Art Assets) | 640×360 mp4, 10.00s, ~0.4 MB; loops on the detail-page header. Poster: reuse the Cover Art |
...
seconds, under 1 MB**, with a poster image. `preview-video-640x360.mp4` is scripted real
gameplay (miss-shake → finds → ticker → heat colors), captured headlessly at exactly
640×360 and encoded with libx264 (`-crf 14`, `-movflags +faststart`). To re-record: drive
the game under Playwright with `recordVideo: { size: { 640, 360 } }`, trim with ffmpeg,
and re-check the size cap.
