...
Regions are pure data (`docs/mappack-contract.md`, types in `shared/mappack.ts`). A pack carries stable feature ids, display names, aliases, and projection metadata; shipped packs are `us-states` (pre-projected Albers USA from public-domain `us-atlas`), `europe`, `asia`, `africa`, `south-america` (pre-projected conic conformal from public-domain Natural Earth data via `world-atlas`), and `continents` + `oceans` (a shared equirectangular world frame; continents group every atlas country, oceans are hand-traced coarse rings). The renderer plots coordinates directly — no reprojection — and pan/zoom plus auto-framing covers small-region hit areas. Adding a continent means adding a builder script's two artifacts and a manifest entry; no engine changes. Scripts: `scripts/fetch-mappacks.mjs`, `scripts/build-*.mjs` (shared math in `scripts/lib/topo-utils.mjs`, `continent-builder.mjs`, and `world-builder.mjs`, plus `poi-builder.mjs` which turns pure data tables in `scripts/data/` into dot-target packs like world-landmarks), validated by `scripts/validate-mappack.mjs` (`npm run packs:validate`), with contract/unit coverage under `tests/` (`npm test`).
