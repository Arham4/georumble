...
/**
 * Answers one question: how many simultaneously live rooms are we willing to
 * host? null means unlimited (local dev, or a plan whose ceiling we cannot
 * read). Only NEW rooms consult it; running games always finish.
 */
...
// cannot silently admit unbounded rooms. LIMITS_KV overrides any of this
...
export class CloudflareRoomLimit implements RoomLimitSource {
  ...
  async limit(): Promise<number | null> {
    ...
    // Collapse concurrent callers onto one refresh; admissions burst at room
    // creation and every player in a launching voice channel arrives at once.
